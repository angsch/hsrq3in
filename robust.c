...
#define T(i,j) T[(i) + (j) * (size_t)ldT]
    ...
    if (CI(0,0) != 0.0 || CI(1,1) != 0.0) {
        ...
        *scale = (*scale) * beta;
        ...
        {
        // C11 is real-valued. Update system matrix.
        double beta1 = protect_update_scalar(CR(1,0), CR(0,1), CR(1,1));
        double beta2 = protect_update_scalar(CI(1,0), CI(0,1), CR(1,1));
        double beta3 = protect_mul(CI(1,0), CR(0,1));
        double beta4 = protect_mul(CR(1,0), CI(0,1));
        double beta = fmin(fmin(beta1, beta2), fmin(beta3, beta4));
        // Scale C and the rhs. Note that CR, CI are local copies and the
        // system matrix remains unaltered.
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 3; j++) {
                CR(i,j) = CR(i,j) * beta;
                CI(i,j) = CI(i,j) * beta;
        ...
        *scale = (*scale) * beta;
        ...
        // Investigate multiplications and apply most constraining scaling.
        beta1 = protect_mul(CR(1,0), CI(0,2));
        beta2 = protect_mul(CI(1,0), CI(0,2));
        beta3 = protect_mul(CI(1,0), CR(0,2));
        beta4 = protect_mul(CR(1,0), CI(0,2));
        beta = fmin(fmin(beta1, beta2), fmin(beta3, beta4));
        *scale = (*scale) * beta;
        CR(0,2) = CR(0,2) * beta; CI(0,2) = CI(0,2) * beta;
        CR(1,2) = CR(1,2) * beta; CI(1,2) = CI(1,2) * beta;
